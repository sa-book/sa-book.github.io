...
Now that we stored the MCSA dissimilarity matrix in the object mcdist.om, we can use the ?wcKMedRange command to perform a PAM clustering.
...
We then select the 4th column - where the ASW values are stored...

```{r, eval=TRUE, echo=TRUE}
mc.asw <- mc.val[,4]
```

... and the 7th column - where the R2 values are stored:
...
We can now plot the MCSA clusters, here sequences in each sub-graph are ordered by multidimensional scaling calcualted on both channels.
