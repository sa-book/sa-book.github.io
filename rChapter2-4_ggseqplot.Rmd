...
We wrote a little R package named [`{ggseqplot}`](https://github.com/maraab23/ggseqplot) that reproduces sequence plots from [`{TraMineR}`](http://traminer.unige.ch){target="_blank"}’s `seqplot` using [`{ggplot2}`](https://ggplot2.tidyverse.org/){target="_blank"}. These plots are produced on the basis of a sequence object defined with `TraMineR::seqdef`. The package automates the reshaping and plotting of sequence data required to render the plots with [`{ggplot2}`](https://ggplot2.tidyverse.org/){target="_blank"}.
...
[`{ggseqplot}`](https://maraab23.github.io/ggseqplot/index.html) has been written after this book has been published. You can install and load it with the following commands: 
...
For further information we refer to the [`{ggseqplot}` website](https://maraab23.github.io/ggseqplot/index.html) and a [blog post](https://sequenceanalysis.org/2022/08/05/ggseqplot-ggplotify-sequence-plots/) on the website of the Sequence Analysis Association (SAA).

The following section briefly illustrates the package by rendering sequence distribution and index plots.
...
Of course, the labels can also be adjusted according to your need using the regular [`{ggplot2}`](https://ggplot2.tidyverse.org/){target="_blank"} functions. If you want to access the value labels for the x-axis, you can either get them from the sequence object you are plotting (`attributes(partner.child.year.seq)$names`) or by saving your `ggseqdplot` as an object and then extracting the required information from the `data` column `k`. 
...
Of course, the figures produced with [`{ggseqplot}`](https://maraab23.github.io/ggseqplot/index.html)
can be adjusted like any other [`{ggplot2}`](https://ggplot2.tidyverse.org/){target="_blank"} figure

```{r theming, layout="l-body-outset", fig.width=10, fig.height=6.5}
library(hrbrthemes)
...
The following code chunk illustrates how the plot can be fine-tuned in the well-known [`{ggplot2}`](https://ggplot2.tidyverse.org/){target="_blank"} fashion:   

```{r, layout="l-body-outset", fig.width=10, fig.height=6.5}
ggseqiplot(partner.child.year.seq[subset,],
           sortv = rowSums(seqistatd(partner.child.year.seq[subset,])[,7:9])) +
  scale_y_continuous(expand = expansion(add = c(.8, 0))) +
  scale_x_discrete(breaks = c(seq(1,21,2),22),
                     labels = c(seq(18,38,2),39)) +
