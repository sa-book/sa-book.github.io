...
We then recode the theoretical sequence from SPS to STS and overwrite the object that cointained it:
...
We can now compute the dissimilarity between the theoretical sequence and all sequences in the data. Here we use OM with constant substitution costs and `indel` equal to 1. The `theo.seq` object has to be specified as argument to the option `refseq`:
...
We can identify the most frequen sequence by using the ?seqtab command and specify the `idxs` as follows and store it in a separate object, here called mostfreq.seq: 
...
First, we have to compute a dissimilarity matrix, here we use OM with constant substitution costs and `indel` equal to 1. 
...
We display the dissimilarity values for 3 sequences in the sample and the three reference sequences:
