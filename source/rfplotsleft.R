...
###########################################
# rf plots only left plot
###########################################

seqplot.rf.l <- function(seqdata, k=floor(nrow(seqdata)/10), diss, sortv=NULL,
...
seqplot.rf_internal <- function(seqdata, k=floor(nrow(seqdata)/10), diss, sortv=NULL,
  ...
  ###### here is the difference to the original code
  
  ##Correct weights to their original weights (otherwise we use the medoid weights)
  attr(seqtoplot, "weights") <- NULL
  seqIplot(seqtoplot, with.legend=FALSE, sortv=mdsk, yaxis=yaxis, title=main, ...)
