...
#
#  Modified by Ben Bolker to take a box.cex argument.
#  http://ms.mcmaster.ca/~bolker/R/misc/legendx.R
#
#  Modified by Paul Hurtado to take a cex.lab argument.
#  http://pauljhurtado.com/R/legendxx.R
#
...
legend_large_box <-
  function(x, y = NULL, legend, fill = NULL, col = par("col"), border="black",
           ...
           box.cex = c(0.8,0.5), cex.title=cex, cex.lab=cex,
    ...
    if(plot) {
      if (!is.null(title))
        text2(left + w*title.adj, top - ymax, labels = title,
              adj = c(title.adj, 0), cex = cex.title, col = title.col)
      
      text2(xt, yt, labels = legend, adj = adj, cex = cex.lab,
