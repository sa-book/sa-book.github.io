...
We import the data to R using the `read_dta` function from the [`haven`](https://haven.tidyverse.org/index.html) package and inspect the names of the imported variables.
...
The first examples in the book are based on sequences with a reduced alphabet only distinguishing partnership states. The following code generates a data set (`seqvars.partner`) containing the recoded sequence variables using [`dplyr`](https://dplyr.tidyverse.org/index.html).
...
Then we define two vectors storing the long and short labels for the states in the newly defined alphabet. Once that is done, we can define the data as a state sequence object. Most [`TraMineR`](http://traminer.unige.ch/) functions for analyzing sequences require the data to have this format.
